  ...
  const startCamera = async () => {
    try {
      ...
      qrScannerRef.current = new QrScanner(
        ...
        async (result) => {
          try {
            const decoded = JSON.parse(result.data); // QR should contain { plateNumber: "ABC123" }
            const response = await axios.get(
              `/vehicle/plate/${decoded.plateNumber}`
            );
            if (response.data.success) setVehicleData(response.data.vehicle);
            else setCameraError(response.data.message);
          } catch (err: any) {
            setCameraError("Invalid QR or backend error");
          }
          stopCamera();
        },
        { preferredCamera: "environment" }
    ...
    <div className="min-h-screen p-6 bg-background">
      <Card className="p-8">
        <div className="text-center mb-6">
          ...
          <h2 className="text-xl font-bold">Vehicle QR Scanner</h2>
          <p>Scan QR code to get vehicle info</p>
        </div>
      ...
      {vehicleData && (
        <Card className="p-6 mt-6">
          ...
          <p>Plate: {vehicleData.plateNumber}</p>
          ...
          <h4 className="text-md font-semibold mt-4 mb-2">Violations</h4>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              <Camera className="inline w-4 h-4 mr-2" /> QR Code Scanner
              <Button onClick={stopCamera} variant="ghost" className="ml-auto">
                <X />
              </Button>
            </DialogTitle>
          </DialogHeader>
          <video
            ...
            autoPlay
          />
          <Button onClick={stopCamera} className="w-full mt-2">
            Cancel
          </Button>
        </DialogContent>
      </Dialog>
